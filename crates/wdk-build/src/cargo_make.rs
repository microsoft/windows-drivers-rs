...
impl ParseCargoArg for BaseOptions {
    fn parse_cargo_arg(&self) {
        ...
        if self.quiet {
            append_to_env_var(CARGO_MAKE_CARGO_BUILD_TEST_FLAGS_ENV_VAR, "--quiet");
        }
...
impl ParseCargoArg for clap_cargo::Workspace {
    fn parse_cargo_arg(&self) {
        ...
        if self.workspace {
            append_to_env_var(CARGO_MAKE_CARGO_BUILD_TEST_FLAGS_ENV_VAR, "--workspace");
        }
        ...
        if self.all {
            append_to_env_var(CARGO_MAKE_CARGO_BUILD_TEST_FLAGS_ENV_VAR, "--all");
        }
    }
}
...
impl ParseCargoArg for clap_cargo::Features {
    fn parse_cargo_arg(&self) {
        if self.all_features {
            append_to_env_var(CARGO_MAKE_CARGO_BUILD_TEST_FLAGS_ENV_VAR, "--all-features");
        }
    ...
    fn parse_cargo_arg(&self) {
            ...
            .as_ref()
        {
            ...
            _ => {
                ...
                if self.release {
                    ...
                    append_to_env_var(CARGO_MAKE_CARGO_BUILD_TEST_FLAGS_ENV_VAR, "--release");
        ...
        if let Some(timings_option) = &self.timings {
            timings_option.as_ref().map_or_else(
                || {
                    append_to_env_var(CARGO_MAKE_CARGO_BUILD_TEST_FLAGS_ENV_VAR, "--timings");
                },
                |timings_value| {
                    append_to_env_var(
                        CARGO_MAKE_CARGO_BUILD_TEST_FLAGS_ENV_VAR,
...
impl ParseCargoArg for ManifestOptions {
    fn parse_cargo_arg(&self) {
        if self.frozen {
            append_to_env_var(CARGO_MAKE_CARGO_BUILD_TEST_FLAGS_ENV_VAR, "--frozen");
        }

        if self.locked {
            append_to_env_var(CARGO_MAKE_CARGO_BUILD_TEST_FLAGS_ENV_VAR, "--locked");
        }

        if self.offline {
            append_to_env_var(CARGO_MAKE_CARGO_BUILD_TEST_FLAGS_ENV_VAR, "--offline");
        }
    }
}
...
fn append_to_env_var<S: AsRef<str>>(env_var_name: S, string_to_append: S) {
