...
impl<'a> PackageTask<'a> {
    ...
    pub fn new(params: PackageTaskParams<'a>, wdk_build: &'a WdkBuild, fs: &'a Fs) -> Self {
    ...
    fn run_stampinf(&self) -> Result<(), PackageTaskError> {
        ...
        if let Err(e) = CommandExec::run("stampinf", &args, None, None) {
    ...
    fn run_inf2cat(&self) -> Result<(), PackageTaskError> {
        ...
        if let Err(e) = CommandExec::run("inf2cat", &args, None, None) {
    ...
    fn is_self_signed_certificate_in_store() -> Result<bool, PackageTaskError> {
        ...
        match CommandExec::run("certmgr.exe", &args, None, None) {
    ...
    fn create_self_signed_cert_in_store(&self) -> Result<(), PackageTaskError> {
        ...
        if let Err(e) = CommandExec::run("makecert", &args, None, None) {
    ...
    fn create_cert_file_from_store(&self) -> Result<(), PackageTaskError> {
        ...
        if let Err(e) = CommandExec::run("certmgr.exe", &args, None, None) {
    ...
    fn run_signtool_sign(
        ...
        if let Err(e) = CommandExec::run("signtool", &args, None, None) {
    ...
    fn run_infverif(&self) -> Result<(), PackageTaskError> {
        ...
        if let Err(e) = CommandExec::run("infverif", &args, None, None) {
...
mod tests {
    ...
    fn new_succeeds_for_valid_args() {
        ...
        let fs = Fs::default();
        let task = PackageTask::new(package_task_params, &wdk_build, &fs);
    ...
    fn new_panics_when_target_dir_is_not_absolute() {
        ...
        PackageTask::new(package_task_params, &wdk_build, &fs);
    }
        ...
        for (name, env_val, expect_skip_v) in scenarios {
            let result =
                crate::test_utils::with_env(&[(STAMPINF_VERSION_ENV_VAR, env_val)], || {
                    ...
                    let task = PackageTask::new(params, &wdk_build, &fs);
                    task.run_stampinf()
                });
