use std::{collections::HashMap, io::Error, path::PathBuf, process::{ExitStatus, Output}, result::Result::Ok};
...
pub fn given_a_simple_driver_project_when_default_values_are_provided_then_it_builds_successfully() {
    ...
    let cargo_toml_metadata = serde_json::from_str::<cargo_metadata::Metadata>(&cargo_toml_metadata).unwrap();

    // Mock expectation for init
    let cargo_toml_metadata_clone = cargo_toml_metadata.clone();
    wdk_build_provider_mock.expect_get_cargo_metadata_at_path().once().returning(move |_| Ok(cargo_toml_metadata_clone.clone()));
    // working dir
    let cwd_clone = cwd.clone();
    let expected_cwd = cwd.clone();
    fs_provider_mock.expect_canonicalize_path().withf(move |d: &PathBuf| d.eq(&expected_cwd)).once().returning( move |_| Ok(cwd_clone.to_owned()));
    ...
    fs_provider_mock.expect_canonicalize_path().withf(move |d: &PathBuf| d.eq(&expected_workspace_root)).once().returning( move |_| Ok(workspace_root.clone().into()));
    ...
    run_command_mock.expect_run().withf(move |command: &str, args: &[&str], _env_vars: &Option<&HashMap<&str, &str>>| -> bool { 
        command == expected_cargo_command && args == expected_cargo_build_args
    }).once().returning(move |_, _, _| Ok(expected_output.clone()));
    ...
    fs_provider_mock.expect_exists().with(eq(expected_inx_file_path)).once().returning(|_| true);
    ...
    fs_provider_mock.expect_rename().with(eq(expected_src_driver_dll_path), eq(expected_src_driver_sys_path.clone())).once().returning(|_, _| Ok(()));

    // copy .sys to package directory
    let expected_dest_driver_binary_path = expected_final_package_dir_path.clone().join(format!("{}.sys", expected_driver_name_underscored));
    let expected_src_driver_binary_path = expected_src_driver_sys_path.clone();
    let mock_non_zero_bytes_copied_size = 1000;
    fs_provider_mock.expect_copy().with(eq(expected_src_driver_binary_path), eq(expected_dest_driver_binary_path.clone())).once().returning(move |_, _| Ok(mock_non_zero_bytes_copied_size));

    // copy pdb file to package directory
    let expected_src_driver_pdb_path = expected_target_dir.join(format!("{}.pdb", expected_driver_name_underscored));
    let expected_dest_driver_pdb_path = expected_final_package_dir_path.clone().join(format!("{}.pdb", expected_driver_name_underscored));
    fs_provider_mock.expect_copy().with(eq(expected_src_driver_pdb_path), eq(expected_dest_driver_pdb_path.clone())).once().returning(move |_, _| Ok(mock_non_zero_bytes_copied_size));

    // copy inx file to package directory
    let expected_src_driver_inx_path = cwd.join(format!("{}.inx", expected_driver_name_underscored));
    let expected_dest_driver_inf_path = expected_final_package_dir_path.clone().join(format!("{}.inf", expected_driver_name_underscored));
    fs_provider_mock.expect_copy().with(eq(expected_src_driver_inx_path), eq(expected_dest_driver_inf_path.clone())).once().returning(move |_, _| Ok(mock_non_zero_bytes_copied_size));

    // copy map file to package directory
    let expected_src_driver_map_path = expected_target_dir.join("deps").join(format!("{}.map", expected_driver_name_underscored));
    let expected_dest_driver_map_path = expected_final_package_dir_path.clone().join(format!("{}.map", expected_driver_name_underscored));
    fs_provider_mock.expect_copy().with(eq(expected_src_driver_map_path), eq(expected_dest_driver_map_path.clone())).once().returning(move |_, _| Ok(mock_non_zero_bytes_copied_size));
    ...
    let expected_inf2cat_args: Vec<String> = vec![format!("/driver:{}", expected_final_package_dir_path.to_string_lossy()), "/os:10_x64".to_string(), "/uselocaltime".to_string()];
    let expected_output = output.clone();
    run_command_mock.expect_run().withf(move |command: &str, args: &[&str], _env_vars: &Option<&HashMap<&str, &str>>| -> bool { 
    ...
    let expected_makecert_args: Vec<String> = vec!["-r".to_string(), "-pe".to_string(), "-a".to_string(), "SHA256".to_string(),  "-eku".to_string(), "1.3.6.1.5.5.7.3.3".to_string(), "-ss".to_string(), "WDRTestCertStore".to_string(), "-n".to_string(), "CN=WDRLocalTestCert".to_string(),  expected_src_driver_cert_path.to_string_lossy().to_string()];
    let expected_output = output.clone();
    run_command_mock.expect_run().withf(move |command: &str, args: &[&str], _env_vars: &Option<&HashMap<&str, &str>>| -> bool { 
    ...
    let expected_dest_driver_cert_path = expected_final_package_dir_path.clone().join("WDRLocalTestCert.cer");
    fs_provider_mock.expect_copy().with(eq(expected_src_cert_file_path), eq(expected_dest_driver_cert_path.clone())).once().returning(move |_, _| Ok(mock_non_zero_bytes_copied_size));

    // sign driver binary using signtool
    let expected_signtool_command: &'static str = "signtool";
    let expected_dest_driver_binary_path = expected_final_package_dir_path.clone().join(format!("{}.sys", expected_driver_name_underscored));
    let expected_signtool_args: Vec<String> = vec!["sign".to_string(), "/v".to_string(), "/s".to_string(), "WDRTestCertStore".to_string(), "/n".to_string(), "WDRLocalTestCert".to_string(), "/t".to_string(), "http://timestamp.digicert.com".to_string(), "/fd".to_string(), "SHA256".to_string(), expected_dest_driver_binary_path.to_string_lossy().to_string()];
    let expected_output = output.clone();
    run_command_mock.expect_run().withf(move |command: &str, args: &[&str], _env_vars: &Option<&HashMap<&str, &str>>| -> bool { 
    ...
    let expected_dest_driver_binary_path = expected_final_package_dir_path.clone().join(format!("{}.sys", expected_driver_name_underscored));
    let expected_signtool_verify_args: Vec<String> = vec!["verify".to_string(), "/v".to_string(), "/pa".to_string(), expected_dest_driver_binary_path.to_string_lossy().to_string()];
    let expected_output = output.clone();
    run_command_mock.expect_run().withf(move |command: &str, args: &[&str], _env_vars: &Option<&HashMap<&str, &str>>| -> bool { 
    ...
    let expected_signtool_command: &'static str = "signtool";
    let expected_dest_driver_cat_file_path = expected_final_package_dir_path.clone().join(format!("{}.cat", expected_driver_name_underscored));
    let expected_signtool_args: Vec<String> = vec!["sign".to_string(), "/v".to_string(), "/s".to_string(), "WDRTestCertStore".to_string(), "/n".to_string(), "WDRLocalTestCert".to_string(), "/t".to_string(), "http://timestamp.digicert.com".to_string(), "/fd".to_string(), "SHA256".to_string(), expected_dest_driver_cat_file_path.to_string_lossy().to_string()];
    let expected_output = output.clone();
    run_command_mock.expect_run().withf(move |command: &str, args: &[&str], _env_vars: &Option<&HashMap<&str, &str>>| -> bool { 
        command == expected_signtool_command && args == expected_signtool_args
    }).once().returning(move |_, _, _| Ok(expected_output.to_owned()));
    ...
    let expected_dest_driver_cat_file_path = expected_final_package_dir_path.clone().join(format!("{}.cat", expected_driver_name_underscored));
    let expected_signtool_verify_args: Vec<String> = vec!["verify".to_string(), "/v".to_string(), "/pa".to_string(), expected_dest_driver_cat_file_path.to_string_lossy().to_string()];
    let expected_output = output.clone();
    run_command_mock.expect_run().withf(move |command: &str, args: &[&str], _env_vars: &Option<&HashMap<&str, &str>>| -> bool { 
        command == expected_signtool_command && args == expected_signtool_verify_args
    }).once().returning(move |_, _, _| Ok(expected_output.to_owned()));
    ...
    wdk_build_provider_mock.expect_detect_wdk_build_number().once().returning(|| Ok(26100));

    // Act
    let def_package_project = PackageProjectArgs { cwd: cwd, profile, target_arch, sample_class };
    let package_project = PackageAction::new(def_package_project.cwd, 
...
fn get_cargo_metadata(root_dir: &PathBuf, package_list: Vec<String>, workspace_member_list: Vec<String>, metadata: Option<String>) -> String {
    ...
    {{
        ...
        "metadata": {},
        "version": 1
    }}"#, root_dir.join("target").to_string_lossy().escape_default(), root_dir.to_string_lossy().escape_default(), package_list.join(", "),
    // Require quotes around each member
    workspace_member_list.iter().map(|s| format!("\"{}\"", s)).collect::<Vec<String>>().join(", "), metadata_section)
}

fn get_cargo_metadata_package(root_dir: &PathBuf, default_package_name: &str, default_package_version: &str, metadata: Option<String>) -> (String, String) {
    let package_id = format!("path+file:///{}#{}@{}", root_dir.to_string_lossy().escape_default(), default_package_name, default_package_version);
    let metadata_section = match metadata {
        Some(metadata) => metadata,
        None => String::from("null")
    };
    (package_id, 
        ...
        "#, default_package_name, default_package_version,
        format!("path+file:///{}#{}@{}", root_dir.to_string_lossy().escape_default(), default_package_name, default_package_version),
        default_package_name, root_dir.join("src").join("main.rs").to_string_lossy().escape_default(), 
        root_dir.join("Cargo.toml").to_string_lossy().escape_default(), metadata_section)
    )
}

fn get_cargo_metadata_wdk_metadata(driver_type: &str, kmdf_version_major: u8, target_kmdf_version_minor: u8) -> String {
    format!(r#"
        {{
            "wdk": {{
                "driver-model": {{
...
pub fn given_a_simple_driver_project_when_default_values_are_provided_then_it_builds_successfully_simplified() {
    ...
    let package_project = TestPackageAction::new(cwd.clone(), profile.clone(), target_arch.clone(), sample_class);
    let package_project_action = package_project
        ...
        .expect_dir_exists(driver_name, &cwd, true)
...
// Output argument in any method means to override return output from default success with no stdout/stderr
trait TestSetupPackageExpectations {
    ...
    fn expect_dir_exists(self, driver_name: &str, driver_dir: &PathBuf, does_exist: bool) -> Self;
    ...
    fn expect_inx_file_exists(self, driver_name: &str, driver_dir: &PathBuf, does_exist: bool) -> Self;
    fn expect_rename_driver_binary_dll_to_sys(self, driver_name: &str, driver_dir: &PathBuf) -> Self;
    fn expect_copy_driver_binary_sys_to_package_folder(self, driver_name: &str, driver_dir: &PathBuf, is_success: bool) -> Self;
    fn expect_copy_pdb_file_to_package_folder(self, driver_name: &str, driver_dir: &PathBuf, is_success: bool) -> Self;
    fn expect_copy_inx_file_to_package_folder(self, driver_name: &str, driver_dir: &PathBuf, is_success: bool) -> Self;
    fn expect_copy_map_file_to_package_folder(self, driver_name: &str, driver_dir: &PathBuf, is_success: bool) -> Self;
    fn expect_copy_self_signed_cert_file_to_package_folder(self, driver_name: &str, driver_dir: &PathBuf, is_success: bool) -> Self;

    fn expect_stampinf(self, driver_name: &str, driver_dir: &PathBuf, override_output: Option<Output>) -> Self;
    fn expect_inf2cat(self, driver_name: &str, driver_dir: &PathBuf,  override_output: Option<Output>) -> Self;
    fn expect_certmgr_exists_check(self, override_output: Option<Output>) -> Self;
    fn expect_certmgr_create_cert_from_store(self, driver_name: &str, driver_dir: &PathBuf,  override_output: Option<Output>) -> Self;
    ...
    fn expect_infverif(self, driver_name: &str, driver_dir: &PathBuf, driver_type: &str, override_output: Option<Output>) -> Self;
...
impl TestPackageAction {
    fn new(cwd: PathBuf, profile: String, target_arch: String, sample_class: bool) -> Self {
        ...
        Self { cwd, profile, target_arch, sample_class, mock_run_command, mock_wdk_build_provider, mock_fs_provider, cargo_metadata: None }
    }

    fn set_up_simple_driver_project(mut self, package_metadata: (String, String)) -> impl TestSetupPackageExpectations {
        let cargo_toml_metadata = get_cargo_metadata(&self.cwd, vec![package_metadata.1], vec![package_metadata.0], None);
        // println!("cargo_toml_metadata: {}", cargo_toml_metadata);
        let cargo_toml_metadata = serde_json::from_str::<cargo_metadata::Metadata>(&cargo_toml_metadata).unwrap();
        let cargo_toml_metadata_clone = cargo_toml_metadata.clone();
        self.mock_wdk_build_provider.expect_get_cargo_metadata_at_path().once().returning(move |_| Ok(cargo_toml_metadata_clone.clone()));
        self.cargo_metadata = Some(cargo_toml_metadata.clone());
        self
    }

    fn set_up_workspace_with_multiple_driver_projects(self, workspace_additional_metadata: String, package_metadata_list: Vec<(String, String)>) -> impl TestSetupPackageExpectations {
        self
    }
    
    fn set_up_with_custom_toml(self, cargo_toml_metadata: Metadata) -> impl TestSetupPackageExpectations {
        self
    }
}
...
impl TestSetupPackageExpectations for TestPackageAction {
    fn expect_path_canonicalization_cwd(mut self) -> Self {
        ...
        let expected_cwd = cwd.clone();
        self.mock_fs_provider.expect_canonicalize_path().withf(move |d: &PathBuf| d.eq(&expected_cwd)).once().returning( move |_| Ok(cwd.to_owned()));
        self
    }

    fn expect_path_canonicalization_workspace_root(mut self) -> Self {
        let workspace_root_dir: PathBuf = self.cargo_metadata.as_ref().unwrap().workspace_root.clone().into();
        let expected_workspace_root_dir = workspace_root_dir.clone();
        self.mock_fs_provider.expect_canonicalize_path().withf(move |d: &PathBuf| d.eq(&expected_workspace_root_dir)).once().returning( move |_| Ok(workspace_root_dir.to_owned()));
        self
    }

    fn expect_path_canonicalization_package_root(mut self) -> Self {
        self.cargo_metadata.as_ref().unwrap().workspace_packages().iter().for_each(|package| {
            ...
            self.mock_fs_provider.expect_canonicalize_path().withf(move |d: &PathBuf| d.eq(&expected_package_root_path)).once().returning( move |_| Ok(package_root_path.to_owned()));
        });
        self
    }
    ...
    fn expect_dir_created(self, driver_name: &str, cwd: &PathBuf, created: bool) -> Self {
        todo!()
    }
    ...
    fn expect_cargo_build(mut self,  driver_name: &str, override_output: Option<Output>) -> Self {
        // cargo build on the package
        let expected_cargo_command: &'static str = "cargo";
        let expected_cargo_build_args: Vec<String> = vec!["build", "-v", "-p", &driver_name].into_iter().map(|s| s.to_string()).collect();
    ...
    fn expect_inx_file_exists(mut self, driver_name: &str, driver_dir: &PathBuf, does_exist: bool) -> Self {
        let expected_driver_name_underscored = driver_name.replace("-", "_");
        let expected_inx_file_path = driver_dir.join(format!("{}.inx", expected_driver_name_underscored));
        self.mock_fs_provider.expect_exists().with(eq(expected_inx_file_path)).once().returning(move |_| does_exist);
        self
    }
    ...
    fn expect_rename_driver_binary_dll_to_sys(mut self, driver_name: &str, driver_dir: &PathBuf) -> Self {
        ...
        let expected_src_driver_dll_path = expected_target_dir.join(format!("{}.dll", expected_driver_name_underscored));
        let expected_src_driver_sys_path = expected_target_dir.join(format!("{}.sys", expected_driver_name_underscored));
        self.mock_fs_provider.expect_rename().with(eq(expected_src_driver_dll_path), eq(expected_src_driver_sys_path.clone())).once().returning(|_, _| Ok(()));
        self
    }
    ...
    fn expect_copy_driver_binary_sys_to_package_folder(mut self, driver_name: &str, driver_dir: &PathBuf, is_success: bool) -> Self {
        ...
        let expected_dest_driver_binary_path = expected_final_package_dir_path.clone().join(format!("{}.sys", expected_driver_name_underscored));
        let expected_src_driver_binary_path = expected_src_driver_sys_path.clone();
        self.mock_fs_provider.expect_copy().with(eq(expected_src_driver_binary_path), eq(expected_dest_driver_binary_path.clone())).once().returning(move |_, _| {
            if is_success { Ok(mock_non_zero_bytes_copied_size) } else { Err(Error::new(std::io::ErrorKind::UnexpectedEof, "copy error")) }
        });
        self
    }
    ...
    fn expect_copy_pdb_file_to_package_folder(mut self, driver_name: &str, driver_dir: &PathBuf, is_success: bool) -> Self {
        ...
        let expected_src_driver_pdb_path = expected_target_dir.join(format!("{}.pdb", expected_driver_name_underscored));
        let expected_dest_driver_pdb_path = expected_final_package_dir_path.clone().join(format!("{}.pdb", expected_driver_name_underscored));
        self.mock_fs_provider.expect_copy().with(eq(expected_src_driver_pdb_path), eq(expected_dest_driver_pdb_path.clone())).once().returning(move |_, _| {
            if is_success { Ok(mock_non_zero_bytes_copied_size) } else { Err(Error::new(std::io::ErrorKind::UnexpectedEof, "copy error")) }
        });
        self
    }
    ...
    fn expect_copy_inx_file_to_package_folder(mut self, driver_name: &str, driver_dir: &PathBuf, is_success: bool) -> Self {
        ...
        // copy inx file to package directory
        let expected_src_driver_inx_path = driver_dir.join(format!("{}.inx", expected_driver_name_underscored));
        let expected_dest_driver_inf_path = expected_final_package_dir_path.clone().join(format!("{}.inf", expected_driver_name_underscored));
        self.mock_fs_provider.expect_copy().with(eq(expected_src_driver_inx_path), eq(expected_dest_driver_inf_path.clone())).once().returning(move |_, _| {
            if is_success { Ok(mock_non_zero_bytes_copied_size) } else { Err(Error::new(std::io::ErrorKind::UnexpectedEof, "copy error")) }
        });
        self
    }
    ...
    fn expect_copy_map_file_to_package_folder(mut self, driver_name: &str, driver_dir: &PathBuf, is_success: bool) -> Self {
        ...
        let expected_src_driver_map_path = expected_target_dir.join("deps").join(format!("{}.map", expected_driver_name_underscored));
        let expected_dest_driver_map_path = expected_final_package_dir_path.clone().join(format!("{}.map", expected_driver_name_underscored));
        self.mock_fs_provider.expect_copy().with(eq(expected_src_driver_map_path), eq(expected_dest_driver_map_path.clone())).once().returning(move |_, _| {
            if is_success { Ok(mock_non_zero_bytes_copied_size) } else { Err(Error::new(std::io::ErrorKind::UnexpectedEof, "copy error")) }
        });
        self
    }
    ...
    fn expect_copy_self_signed_cert_file_to_package_folder(mut self, driver_name: &str, driver_dir: &PathBuf, is_success: bool) -> Self {
        ...
        let expected_src_cert_file_path = expected_target_dir.clone().join("WDRLocalTestCert.cer");
        let expected_dest_driver_cert_path = expected_final_package_dir_path.clone().join("WDRLocalTestCert.cer");
        self.mock_fs_provider.expect_copy().with(eq(expected_src_cert_file_path), eq(expected_dest_driver_cert_path.clone())).once().returning(move |_, _| {
            if is_success { Ok(mock_non_zero_bytes_copied_size) } else { Err(Error::new(std::io::ErrorKind::UnexpectedEof, "copy error")) }
        });
        self
    }
    ...
    fn expect_detect_wdk_build_number(mut self, expected_wdk_build_number: u32) -> Self {
        self.mock_wdk_build_provider.expect_detect_wdk_build_number().once().returning(move || Ok(expected_wdk_build_number));
        self
    }
    
    fn expect_infverif(mut self, driver_name: &str, driver_dir: &PathBuf, driver_type: &str, override_output: Option<Output>) -> Self {
        ...
        let expected_dest_inf_file_path = expected_final_package_dir_path.clone().join(format!("{}.inf", expected_driver_name_underscored));
