    ...
    let wdk_sys_out_dir = match wdk_sys_out_dir.len() {
        ...
        _ => {
            return Err(Error::new(
                Span::call_site(),
                format!(
                    "Expected exactly one instance of wdk-sys in dependency graph when running `cargo check`, found {}",
                    wdk_sys_out_dir.len()
                ),
            ));
        }
...
fn find_wdk_sys_pkg_id() -> Result<PackageId> {
    ...
    if wdk_sys_package_matches.len() != 1 {
        return Err(Error::new(
            Span::call_site(),
            format!(
                "Expected exactly one instance of wdk-sys in dependency graph when running `cargo metadata`, found {}",
