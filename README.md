...
* Binding generation via `bindgen` requires `libclang`. The easiest way to acquire this is via `winget`
  * `winget install -i LLVM.LLVM --version 17.0.6 --force`
    ...
    * LLVM 18 has a bug that causes bindings to fail to generate for ARM64. Continue using LLVM 17 until LLVM 19 comes out with [the fix]([url](https://github.com/llvm/llvm-project/pull/93235)). See [this]([url](https://github.com/rust-lang/rust-bindgen/issues/2842)) for more details.
